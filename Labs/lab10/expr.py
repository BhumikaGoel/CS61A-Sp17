...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        if self.__str__() in env:
            return env[self.__str__()]
        else:
...
class CallExpr(Expr):
    ...
    def __init__(self, operator, operands):
        ...
        self.operator = operator   # an EXPR
        self.operands = operands   #list of EXPR
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        eval_opt = self.operator.eval(env)
        eval_opr = []
        if self.operands:
            for opr in self.operands:
                eval_opr+=[opr.eval(env)]

        return eval_opt.apply(eval_opr)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        for p in self.parameters:
            #if p.__str__() not in env_copy:
            env_copy[p.__str__()] = p
