...
def make_pytunes(username):
    ...
    """
    "*** YOUR CODE HERE ***"
    treec = tree('justin bieber', [ tree('single', [ tree('what do you mean?') ] ) ] )
    treeb = tree('trance', [ tree('darude', [ tree('sandstorm') ] ) ] )
    treea = tree('pop', [ treec, tree('2015 pop mashup') ] )
    maintree = tree(username, [ treea, treeb ] )
    return maintree
# Q8
def num_songs(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    #Recall that there are no empty directories in pyTunes, so all leaves in t are songs.
    #Hint: You can use is_leaf to check whether a given tree is a leaf.
    if is_leaf(t):
        return 1
    else:
        1 + num_songs(branches(t)[0]) + num_songs(branches(t)[1:])
# Q9
