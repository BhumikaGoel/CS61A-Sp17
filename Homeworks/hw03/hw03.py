...
def div_rat(x, y):
    ...
    20
    """
    numer_res = numer(x)*denom(y)
    denom_res = denom(x)*numer(y)
    return make_rat(numer_res, denom_res)
...
def pingpong(n):
    ...
    def count_up(c,k):
        if k == n:
           ...
           return count_down(c-1, k+1)
        else:
           return count_up(c+1,k+1)
...
def build_ones(h, branching_factor=2):
    ...
    if h == 0:
        ...
        return tree(1, [build_ones(h-1, branching_factor) for x in range(branching_factor)])
