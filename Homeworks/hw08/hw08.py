...
def find_duplicates_k(k, lst):
    ...
    for i in range(0, len(lst)):
        if lst[i] in lst_to_set:
            return True
        else:
            if i - k >= 0:
                lst_to_set.remove(lst[i - k])
            lst_to_set.add(lst[i])
...
def make_random_stream(seed, a, c, n):
    ...
    "*** YOUR CODE HERE ***"
    def compute_rest(prev_val):
        val = ((prev_val*a) + c)%n
        return make_random_stream(val, a, c, n)
