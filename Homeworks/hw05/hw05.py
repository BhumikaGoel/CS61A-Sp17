...
def simplify(expr):
    ...
    if expr.is_leaf():
        ...
        op = expr.oper
        left_branch = simplify(expr.__getitem__(0))
        right_branch = simplify(expr.__getitem__(1))

        if op == '+':
            if left_branch == ZERO:
                return right_branch
            elif right_branch == ZERO:
                return left_branch

        if op == '-':
            if left_branch == right_branch:
                return ZERO
            if right_branch == ZERO:
                return left_branch

        if op =='*':
            if left_branch == ZERO or right_branch == ZERO:
                return ZERO
            elif left_branch == ONE:
                return right_branch
            elif right_branch == ONE:
                return left_branch

        else:
            return Expr(op, [left_branch, right_branch])
...
def dsimplify(expr):
    ...
    if expr.is_leaf():
        ...
        op = expr.oper
        left_branch = dsimplify(expr.__getitem__(0))
        right_branch = dsimplify(expr.__getitem__(1))
        expr.__setitem__(0, left_branch)
        expr.__setitem__(1, right_branch)
...
class VendingMachine:
    ...
    def vend(self):
        ...
        change = self.current_deposit - self._productprice
        if self.current_deposit >= self._productprice:
            self.current_deposit = 0
            self.current_stock-=1
            if change:
                return 'Here is your {} and ${} change.'.format(self._productname, change)
            return 'Here is your {}.'.format(self._productname)
        return 'You must deposit ${} more.'.format(-change)

    def restock(self, st_int):
        self.current_stock += st_int
        return 'Current {} stock: {}'.format(self._productname, self.current_stock)

    def deposit(self, dep_int):
        self.current_deposit += dep_int
        if self.current_deposit and self.current_stock == 0:
            return 'Machine is out of stock. Here is your ${}.'.format(self.current_deposit)
